...
Random errors $u_1$ and $u_2$ are independent with $\P(u_i = 1) = \P(u_i = -1) = 1/2$.}
...
\begin{enumerate}
  ...
  \item \trru{Чему равна дисперсия $\Var(\hat\beta_{\text{ols}})$?}
  \tren{Find the variance $\Var(\hat\beta_{\text{ols}})$.}
  \item \trru{Постройте несмещённую оценку $\hat\beta_{\text{best}}$ с наименьшей дисперсией. }
  \tren{Construct an unbiased estimator $\hat\beta_{\text{best}}$ with lowest possible variance.}
  \item \trru{Чему равна дисперсия $\Var(\hat\beta_{\text{best}})$?}
  \tren{What is the variance $\Var(\hat\beta_{\text{best}})$?}
