...
\item $\hy = X\hb$
\item $\SSR = 2$
\item $R^2 = 1 - \frac {\SSR}{TSS} = 0.8.$ $R^2$ высокий, построенная эконометрическая модель хорошо описывает данные
\end{enumerate}
\end{sol}
\end{problem}
...
\begin{sol}
$\sum y_i^2=\sum \hy_i^2+\sum \hat{u}_i^2$, $TSS=ESS+\SSR$,
\end{sol}
\end{problem}
...
\begin{problem}
\trru{Покажите на картинке $TSS$, $ESS$, $\SSR$, $R^2$, $\sCorr(\hy,y)$, $\sCov(\hy,y)$:}
\trru{Show $TSS$, $ESS$, $\SSR$, $R^2$, $\sCorr(\hy,y)$, $\sCov(\hy,y)$ on the picture:}
...
\begin{problem}
\trru{При каких условиях $TSS=ESS+\SSR$?}
\tren{Under which conditions $TSS=ESS+\SSR$?}
...
Для каждого из данных случаев найдите матрицу-шляпницу $H$, проецирующую векторы на пространство столбцов матрицы регрессоров $X$, вектор $\hy$, $\SSR$, $ESS$, $TSS$:}
\tren{We have $n$ observations. 
For each case find the hat-matrix $H$ that projects vectors onto the column space of regressor matrix $X$, vector $\hy$, $\SSR$, $ESS$, $TSS$:}
...
\[
F_T = \frac{(\SSR_A - \SSR_C)/2}{\SSR_C/(n - 3)}
\]
...
\[
F_C = \frac{\SSR_A - \SSR_B}{\SSR_B/(n - 2)}
\]
...
\tren{
Sheep Dolly had a dataset of $n$ observations with Gauss~— Markov assumptions satisfied. 
She cloned every observation once and added each cloned observation after original observation. 
\begin{enumerate}
  \item Describe the error covariance matrix of the augmented dataset.
  \item How the answer in (a) will change if Dolly will clone only the last observation $n$ times?
\end{enumerate}
}
\begin{sol}
\trru{
...
\tren{
  \begin{enumerate}
    \item Error covariance matrix will contain blocks $B$ on the main diagonal
\[
\Var(u) = \begin{pmatrix}
        B & 0 & 0 & \dots \\
        0 & B & 0 & \dots \\
    ...
    \end{pmatrix},
\]
where each block is equal to $B = \begin{pmatrix}
\sigma^2 & \sigma^2 \\
\sigma^2 & \sigma^2 \\    
\end{pmatrix}$.
    \item Error covariance matrix will consist of four blocks: the top left is proportional to the identity matrix,
    bottom right block will have all elements equal to $\sigma^2$ and two other blocks will be zero:
\[
\Var(u) = \begin{pmatrix}
        \sigma^2 \cdot I & 0 \\
        0 & S \\
    \end{pmatrix},
\]
where $I$ is an identity matrix, and all $S_{ij} = \sigma^2$.
\end{enumerate}
}
\end{sol}
\end{problem}
...
\begin{leftbar}
  \tren{For standardized regressors $X$ and centered $y$ ridge regression minimizes the loss function}
  \trru{При стандартизированных регрессорах $X$ и центрированной зависимой переменной гребневая регрессия минимизируют функцию}
  \[
  \loss(\hb) = (y - \hat y)^T (y - \hat y) + \lambda \hb^T \hb, \quad \hat y = X \hb.
  \]
  \trru{Матрица $(X^TX)^{-1}$ представима в виде}
  \tren{The matrix $(X^TX)^{-1}$ can be written as}
  \[
    (X^TX)^{-1} = \begin{pmatrix}
      1/\SSR_1 & -{\hb_{12}}/{\SSR_1} & -{\hb_{13}}/{\SSR_1}\\
      - {\hb_{21}}/{\SSR_2} & 1/\SSR_2 & - {\hb_{23}}/{\SSR_2} \\
      - {\hb_{31}}/{\SSR_3}  & - {\hb_{32}}/{\SSR_3} & 1/\SSR_3 \\
  \end{pmatrix},
  \]
  \tren{where $\SSR_1$ is the residual sum of squares in the regression of $x_1$ onto all other regressors:}
  \trru{где $\SSR_1$ — это сумма квадратов остатков в регрессии $x_1$ на остальные предикторы:}
  \[
  \hat x_1 = \hb_{12} x_2 + \hb_{13} x_3.
  \] 
\end{leftbar}
...
\begin{enumerate}
  ...
  \item What happens to the estimates when $\lambda\to 0$?
\end{enumerate}
...
\begin{enumerate}[resume]
  ...
  \item What happens to the $\E(\hb \mid X)$ and $\Var(\hb \mid X)$ when $\lambda\to\infty$?
  \item What happens to the $\E(\hb \mid X)$ and $\Var(\hb \mid X)$ when $\lambda\to 0$?
\end{enumerate}
}
  ...
  \begin{sol}
    \begin{enumerate}
      \item $\hb = (X^T X + \lambda I)^{-1} X^T y$;
      \item $\lim_{\lambda\to \infty} \hb = (X^TX)^{-1} X^T y = 0$;
      \item $\lim_{\lambda\to 0} \hb = (X^TX)^{-1} X^T y = \hb_{\text{ols}}$;
      \item $\E(\hb \mid X) = (X^T X + \lambda I)^{-1} X^T X\beta$;
      \item $\Var(\hb \mid X) = \sigma^2 (X^T X + \lambda I)^{-1} X^T X (X^T X + \lambda I)^{-1}$;
      \tren{\item If $\lambda \to \infty$  then $\E(\hb \mid X) \to 0$ and $\Var(\hb \mid X) \to 0$;
      \item If $\lambda \to 0$  then $\E(\hb \mid X) \to \beta$ and $\Var(\hb \mid X) \to \sigma^2 (X^TX)^{-1}$;}
      \trru{\item Если $\lambda \to \infty$,  то $\E(\hb \mid X) \to 0$ и $\Var(\hb \mid X) \to 0$;
      \item Если $\lambda \to 0$ то $\E(\hb \mid X) \to \beta$ и $\Var(\hb \mid X) \to \sigma^2 (X^TX)^{-1}$;}
    \end{enumerate}
  \end{sol}  
\end{problem}
...
\trru{Все регрессоры $X$ уже стандартизированы, зависимая переменная $y$ предварительно центрирована.
  ...
  \begin{sol}
    \tren{One can add $y^+ = 0$, $X^+ = \sqrt{\lambda} I$.}
    \trru{Можно добавить, например, $y^+ = 0$, $X^+ = \sqrt{\lambda} I$.}
  \end{sol}  
\end{problem}

\begin{problem}
  \trru{Матрица $X$ содержит $100$ строк и $4$ столбца: столбец из единиц, столбцы $a$, $b$ и $c$.}
  \tren{Matrix $X$ has $100$ rows and $4$ columns: column of ones, columns $a$, $b$ and $c$.}
\[
1000 \cdot (X^TX)^{-1} = \begin{pmatrix}
  58.2 & 1.6 & 0.7 & 15.2 \\
   & 16.5 & -7.7 & -0.7 \\
   & & 23.6 & -13.1 \\
   & & & 13.1 \\
...
\]
\[
\sum(a_i - \bar a)^2 = 100, \quad \sum(b_i - \bar b)^2 = 160,  \quad \sum(c_i - \bar c)^2 = 240.
\]

\trru{С помощью МНК построили регрессию} \tren{Regression with OLS gives us}
\[
\hat y_i = 2 + 3 a_i - 5 b_i + 6 c_i.
\]
\trru{Сумма квадратов остатков равна $\SSR = 80$, а общая сумма квадратов равна $\SST = 100$.

\begin{enumerate}
  \item Постройте 95\% доверительные интервалы для коэффициентов $\hb_a$, $\hb_b$ и $\hb_c$ в рамках классических предпосылок теоремы Гаусса~— Маркова и нормальных ошибок.
  \item Найдите МНК-оценки коэффициентов в регрессии $\hat a_i = \hat \gamma_1 + \hat\gamma_b b_i + \hat\gamma_c c_i$ и величины $\SSR_a$, $R^2_a$ для этой регрессии.  
  \item Найдите МНК-оценки коэффициентов в регрессии $\hat b_i = \hat \nu_1 + \hat\nu_a a_i + \hat\nu_c c_i$ и величины $\SSR_b$, $R^2_b$ для этой регрессии.  
  \item Найдите МНК-оценки коэффициентов в регрессии $\hat c_i = \hat \delta_1 + \hat\delta_a a_i + \hat\delta_b b_i$ и величины $\SSR_c$, $R^2_c$  для этой регрессии.  
  \item Найдите коэффициенты вздутия дисперсия $\VIF_a$, $\VIF_b$, $\VIF_c$.
\end{enumerate}
}

\tren{Sum of squared residuals is $\SSR = 80$, total sum of squares is $\SST = 100$
\begin{enumerate}
  \item Find the 95\% confidence intervals for the coefficients $\hb_a$, $\hb_b$ and $\hb_c$ under classic Gauss~— Markov assumptions and normal random errors.
  \item Find OLS estimates of coefficents in regression $\hat a_i = \hat \gamma_1 + \hat\gamma_b b_i + \hat\gamma_c c_i$ and $\SSR_a$, $R^2_a$ for this regression.  
  \item Find OLS estimates of coefficents in regression  $\hat b_i = \hat \nu_1 + \hat\nu_a a_i + \hat\nu_c c_i$ and $\SSR_b$, $R^2_b$ for this regression.  
  \item Find OLS estimates of coefficents in regression  $\hat c_i = \hat \delta_1 + \hat\delta_a a_i + \hat\delta_b b_i$ and $\SSR_c$, $R^2_c$  for this regression.  
  \item Find the variance inflation factors $\VIF_a$, $\VIF_b$, $\VIF_c$.
\end{enumerate}
}
...
\begin{problem}
  \tren{Consider the model $y = X\beta + u$.
  There are two centered regressors in the matrix $X$, variable $a$ and variable $b$,
  $\VIF_{a} = 10$, $\SST_a = 125$, $\SST_b = 5$.

  \begin{enumerate}
    \item Find possible values of sample correlation between $a$ and $b$. 
    \item Find $(X^TX)^{-1}$.
  \end{enumerate}}

  \trru{Рассмотрим модель $y = X\beta + u$.
  Матрица $X$ содержит два центрированных регрессора $a$ и $b$,
  $\VIF_{a} = 10$, $\SST_a = 125$, $\SST_b = 5$.

  \begin{enumerate}
    \item Найдите возможные значения выборочной корреляции $a$ и $b$. 
    \item Найдите $(X^TX)^{-1}$.
  \end{enumerate}}
  ...
  \begin{sol}
    \begin{enumerate}
      \item $R_{ab} = \pm 3/ \sqrt{10}$;
      \item 
      \[
      (X^TX)^{-1} = \frac{1}{1 - R^2_{ab}}\begin{pmatrix}
        1/\SST_{a} & -R_{ab} / \sqrt{\SST_a \SST_b} \\
        -R_{ab} / \sqrt{\SST_a \SST_b} &  1/\SST_{b} \\
      \end{pmatrix}
      \]
    \end{enumerate}
  \end{sol}  
...
\begin{problem}
  \trru{Есть два совершенно одинаковых предварительно стандартизированных регрессоров $x$ и $x$, а зависимая переменная $y$ предварительно центрирована.
  ...
  \]
  \begin{enumerate}
    \item Найдите оптимальное значение оценок $\hb_1$ и $\hb_2$ при фиксированном $\lambda$.
    \item Что происходит с оценками при $\lambda\to\infty$?
    \item Что происходит с суммой $\hb_1 + \hb_2$ при $\lambda\to 0$?
  \end{enumerate}
  }
  \tren{We have two absolutely identical preliminary standardized regressors $x$ and $x$. 
  ...
  \begin{sol}
    \begin{enumerate}
      \item $\hb_1 = \hb_2 = 0.5 (x^T x + \lambda / 2)^{-1} x^T y$;
      \item $\hb_j \to 0$;
      \item $\hb_j \to \hb_{\text{ols}} / 2$, $\hat y = \hb_{\text{ols}} x$.
    \end{enumerate}
  \end{sol}  
\end{problem}
...
\begin{problem}
\tren{You know the SVD of $X = UDV^T$.

Find the SVD of $X^TX$, $(X^TX)^{-1}$, $(X^TX)^{-1}X^T$,  $X(X^TX)^{-1}X^T$.
}
\trru{Вы знаете сингулярное разложение матрицы $X = UDV^T$.

Найдите сингулярное разложение для матриц $X^TX$, $(X^TX)^{-1}$, $(X^TX)^{-1}X^T$,  $X(X^TX)^{-1}X^T$.
}
  \begin{sol}
  \end{sol}  
\end{problem}
...
\begin{problem}
\tren{The matrix $X$ has size $n\times 3$. 
All columns of $X$ are standardized, ie they have zero mean and unit sample variance. 
You know the SVD of $X = UDV^T$ with $\diag(D) = (3, 2, 1)$.

Find the SVD of sample correlation matrix of columns of $X$, its eigenvalues and eigenvectors.}
\trru{Матрица $X$ имеет размер $n\times 3$.
Все столбцы матрицы $X$ стандартизированы, то есть имеют нулевое среднее и единичную выборочную дисперсию. 

Вы знаете сингулярное разложение матрицы $X = UDV^T$ и $\diag(D) = (3, 2, 1)$.

Найдите сингулярное разложение выборочной корреляционной матрицы столбцов $X$, её собственные значения и собственные числа.}
  \begin{sol}
  \end{sol}  
\end{problem}
...
\begin{problem}
  \trru{Известно, что $U^TU= I$, где $I$ — единичная матрица. 
  Докажите или утверждения или приведите контр-пример.}
  \tren{We know that $U^TU = I$, where $I$ is an identity matrix.
  Prove the following statements or provide a counter-example. }
  \begin{enumerate}
    \item $UU^T = I$;
    \item \trru{Длина векторов $x$ и $Ux$ всегда одинаковая.} 
    \tren{The lenght of vectors $x$ and $Ux$ is always the same. }
    \item \trru{Равные скалярные произведения} \tren{Scalar products are equal, }
     $\scalp{x, y} = \scalp{Ux, Uy}$.
    \item \trru{Косинус угла между векторами $x$ и $y$ такой же, как косинус угла между векторами $Ux$ и $Uy$.}
  \tren{The cosine of the angle between $x$ and $y$ is equal to the cosine of angle between $Ux$ and $Uy$.}  
  \end{enumerate}
  \begin{sol}
    $(Ux)^T Ux=x^TU^TUx=x^Tx$, $(Ux)^TUy=x^TU^TUy=x^Ty$.
  \end{sol}
  \end{problem}
  
  
  \begin{problem}
  \trru{Найдите SVD-разложение матриц}
  \tren{Find the SVD-decomposition of matrices}
  \begin{enumerate}
    \item
  $X = \begin{pmatrix}
  0 & -7 \\
...
\begin{problem}
  \tren{The $k\times k$ matrix $A$ is constant symmetric positive definite and its highest eigenvalues are $5$ and $2$. 
  
  \begin{enumerate}
    \item Find $d(v^TAv)$ and $d(v^TAv/v^Tv)$.
    \item Solve the optimization problem $\max_v v^T A v$ where $v$ is $k\times 1$ vector with $\norm{v} = 1$
    
    Hint: you may use the differential from (a) or Lagrange multiplier method. 

    \item Solve the optimization problem $\max_w w^T Aw$ where $w$ is $k\times 1$ vector with $\norm{w}=1$ and $w\perp v$.
  \end{enumerate}
  }
  \trru{Матрица $A$ размера $k\times k$ постоянна, положительно определена симметрична, а её наибольшие собственные числа равны  $5$ и $2$. 
  
  \begin{enumerate}
    \item Найдите $d(v^TAv)$ и $d(v^TAv/v^Tv)$.
    \item Найдите максимум $\max_v v^T A v$, где вектор $v$ имеет размер $k\times 1$ и длину $\norm{v} = 1$
    
    Подсказка: можно использовать дифференциал из пункта (а) или метод множителей Лагранжа.

    \item Найдите максимум $\max_w w^T Aw$, где вектор $w$ имеет размер $k\times 1$, длина $\norm{w}=1$ и $w\perp v$.
  \end{enumerate}
  }
  \begin{sol}
  \end{sol}  
...
\begin{problem}
\tren{The matrix $X$ has SVD $X = UDV^T$. The columns of $U$ are $u_1$, $u_2$, \ldots, $u_n$.
Singular values of the diagonal of $D$ are sorted from highest to lowest by absolute value, $d_{11} = 5$, $d_{22} = 3$. 
The columns of $V$ are $v_1$, $v_2$, \ldots, $v_k$.

\begin{enumerate}
  \item Find $X \cdot v_1$, $X \cdot v_2$. 
  \item Solve the optimization problem $\max_w \norm{Xw}$ where $w$ is $k\times 1$ vector with $\norm{w} = 1$.
  \item Solve the optimization problem $\max_z \norm{Xz}$ where $z$ is $k\times 1$ vector with $\norm{z}=1$ and $z\perp w$.
\end{enumerate}
}
\trru{Матрица $X$ имеет сингулярное разложение $X = UDV^T$. 
Столбцы матрицы $U$ равны $u_1$, $u_2$, \ldots, $u_n$.
Сингулярные значения отсортированы на диагонали матрицы $D$ от большего к меньшего по модулю, $d_{11} = 5$, $d_{22}= 3$.
Столбцы матрицы $V$ равны $v_1$, $v_2$, \ldots, $v_k$.

\begin{enumerate}
  \item Найдите $X \cdot v_1$, $X \cdot v_2$. 
  \item Найдите максимум $\max_w \norm{Xw}$, где вектор $w$ имеет размер $k\times 1$ и длину $\norm{w} = 1$.  
  \item Найдите максимум $\max_z \norm{Xz}$, где вектор $z$ имеет размер $k\times 1$ и длину $\norm{z}=1$ и $z\perp w$.
\end{enumerate}
}
  \begin{sol}
  \end{sol}  
...
\section{\trru{Хоббит}\tren{Hobbit}}

\begin{problem}
\trru{
Случайная величина $u$ имеет стандартное нормальное распределение $\cN(0, 1)$ и $w = \max\{u + s, 0\}$, где $s$ — некоторая константа.

\begin{enumerate}
  \item Выразите функцию $\lambda(s) = \E(u \mid u + s > 0)$ с помощи стандартной нормальной функции плотности и функции распределения.
  \item Выразите $\P(w > 0)$, $\E(w)$ и $\E(w \mid w > 0)$ с помощью стандартной нормальной функции плотности или функции распределения.
\end{enumerate}

Функция $\lambda(s)$ называется обратным отношением Миллса.
}
\tren{
  The random variable $u$ has standard normal distribution $\cN(0, 1)$ and $w = \max\{u + s, 0\}$, where $s$ is a constant.
\begin{enumerate}
    \item Write the function $\lambda(s) = \E(u \mid u + s > 0)$ in terms of standard normal density or distribution function.
    \item Write $\P(w > 0)$, $\E(w)$ and $\E(w \mid w > 0)$ in terms of standard normal density or distribution function.
\end{enumerate}

The function $\lambda(s)$ is called inverse Mills ratio.
}
...
\begin{problem}
\tren{Consider the tobit-I model:
\[
\begin{cases}
    y_i^* = x_i^T \beta + u_i, \quad y^* = X^T \beta + u \\
    (u \mid X) \sim \cN(0, \sigma^2 I) \\
    y_i = \max\{y_i^*, 0\} \\
    (x_i, y_i) \text{ are observable for all }i \\
\end{cases}
\]

Find $\E(y_i^* \mid x_i)$, $\E(y_i \mid x_i)$, $\E(y_i \mid x_i, y_i^* > 0)$.
}
\trru{Рассмотрим модель тобит-I:
\[
\begin{cases}
    y_i^* = x_i^T \beta + u_i, \quad y^* = X^T \beta + u \\
    (u \mid X) \sim \cN(0, \sigma^2 I) \\
    y_i = \max\{y_i^*, 0\} \\
    (x_i, y_i) \text{ наблюдаемы при всех }i \\
\end{cases}
\]

Найдите $\E(y_i^* \mid x_i)$, $\E(y_i \mid x_i)$, $\E(y_i \mid x_i, y_i^* > 0)$.
}
  \begin{sol}
  \end{sol}
\end{problem}
...
\begin{problem}
\trru{Рассмотрим логит модель с константой и даже без регрессора,
\[
\P(y_i^* = 1 ) = \Lambda(\beta).
\]
...
\begin{enumerate}
  \item Оцените $\beta$ для выборки $y^* = (1, 1, 0, 1, 0)$.
\end{enumerate}
Злая Колдунья вычёркивает из набора данных все наблюдения с $y_i^* = 0$.
\begin{enumerate}[resume]
  \item Заметим теперь, что у нас появилось два неизвестных параметра, $n$ и $\beta$.
  Найдите оценки обоих параметров методом максимального правдоподобия по выборке $y = (1, 1, 1)$.
\end{enumerate}
}
\tren{
  Consider a logit model with a constant but without other regressors,
\[
\P(y_i^* = 1 ) = \Lambda(\beta).
\]
...
\begin{enumerate}
  \item Estimate $\beta$ for the sample $y^* = (1, 1, 0, 1, 0)$.
\end{enumerate}
Dark Fairy removes all observations with $y_i^* = 0$.
\begin{enumerate}[resume]
  \item Now we have two unknowns: $n$ and $\beta$.
  Estimate both unknown parameters for the truncated sample $y = (1, 1, 1)$.
\end{enumerate}
...
\begin{problem}
\trru{
Величины $y_i^*$ независимы и экспоненциально распределены с неизвестной интенсивностью $\lambda$. 

Злая Колдунья заменяет все наблюдения больше $10$ на $10$.
\begin{enumerate}
  \item Найдите оценку $\lambda$ методом максимального правдоподобия по выборке $y = (5, 10, 6, 10)$.
  \item Найдите оценку $\lambda$ методом максимального правдоподобия по 
  произвольной большой выборке и постройте $95\%$ доверительный интервал. 
\end{enumerate}
}
\tren{
Random variables $y_i^*$ are independnent and exponentially distributed with unknown rate $\lambda$. 

Dark Fairy replaces all observations greater than $10$ by $10$.
\begin{enumerate}
  \item Estimate $\lambda$ by maximal likelihood for the sample $y = (5, 10, 6, 10)$.
  \item Find the estimator of $\lambda$ using maximum likelihood for large arbitrary sample 
  and construct the  $95\%$ confidence interval. 
\end{enumerate}
}
...
Злая Колдунья вычеркивает из набора данных все наблюдения меньше $10$.
\begin{enumerate}
  \item Найдите оценку $\lambda$ методом максимального правдоподобия по выборке $y = (12, 13, 14)$.
  \item Найдите оценку $\lambda$ методом максимального правдоподобия по 
  произвольной большой выборке и постройте $95\%$ доверительный интервал.
  \item Что произойдёт, если по усечённой выборке попытаться оценить размер $n$ исходной выборке? 
\end{enumerate}
}
\tren{
Random variables $y_i^*$ are independnent and exponentially distributed with unknown rate $\lambda$. 

Dark Fairy removes all observations lower than $10$.
\begin{enumerate}
  \item Estimate $\lambda$ by maximal likelihood for the sample $y = (12, 13, 14)$.
  \item Find the estimator of $\lambda$ using maximum likelihood for large arbitrary sample 
  and construct the  $95\%$ confidence interval. 
  \item What happens if one tries to estimate original sample size $n$ using truncated sample?
\end{enumerate}
}
...
\begin{problem}
\trru{
Величины $y_i^*$ независимы и равномерно распределены на отрезке $[0, a]$, где неизвестный параметр $a > 2$.

Злая Колдунья приходит и что-то делает с набором данных :)

\begin{enumerate}
  \item Если возможно, найдите оценку $a$ методом максимального правдоподобия по выборке $y = (1.7, 2, 0.8, 2, 2, 0.9)$,
  если Злая Колдунья заменяет все наблюдения больше $2$ на $2$.
  \item Если возможно, найдите оценку $a$ методом максимального правдоподобия по выборке $y = (1.7, 0.8, 0.9)$,
  если Злая Колдунья удаляет все наблюдения больше $2$.
  \item Если возможно, найдите оценку $a$ методом максимального правдоподобия по выборке $y = (2.7, 2, 3.8, 2, 2, 4.9)$,
  если Злая Колдунья заменяет все наблюдения меньше $2$ на $2$.
  \item Если возможно, найдите оценку $a$ методом максимального правдоподобия по выборке $y = (2.7, 3.8, 4.9)$,
  если Злая Колдунья удаляет все наблюдения меньше $2$.
\end{enumerate}
}
\tren{
Random variables $y_i^*$ are independent and uniformly distributed on $[0, a]$ with unknown $a > 2$.

Dark Fairy arrives and makes her black magic!

\begin{enumerate}
  \item If possible estimate $a$ using maximum likelihood for the sample $y = (1.7, 2, 0.8, 2, 2, 0.9)$,
  if Dark Fairy replaces all observations larger than $2$ by $2$.
  \item If possible estimate $a$ using maximum likelihood for the sample $y = (1.7, 0.8, 0.9)$,
  if Dark Fairy removes all observations larger than $2$.
  \item If possible estimate $a$ using maximum likelihood for the sample $y = (2.7, 2, 3.8, 2, 2, 4.9)$,
  if Dark Fairy replaces all observations less than $2$ by $2$.
  \item If possible estimate $a$ using maximum likelihood for the sample $y = (2.7, 3.8, 4.9)$,
  if Dark Fairy removes all observations less than $2$.
\end{enumerate}
}
...
\begin{problem}
\trru{
Совместный закон распределения $s_i$ и $y_i^*$ имеет вид
...
Злая Колдунья приходит и что-то делает с набором данных :)
После действий Злой Колдуньи Спящая Красавица просыпается и видит набор данных $y = (1, 1, 0, 1, 1, 0)$.

\begin{enumerate}
  \item Если возможно, найдите оценку $a$ методом максимального правдоподобия,
  если Злая Колдунья заменяет $y_i^*$ на $0$ в тех наблюдениях, где $s_i = 1$.
  \item Если возможно, найдите оценку $a$ методом максимального правдоподобия,
  если Злая Колдунья удаляет наблюдения с $s_i = 1$.
  \item Если возможно, найдите оценку $a$ методом максимального правдоподобия,
  если Злая Колдунья ничего не делает.
\end{enumerate}
}
\tren{
The joint distribution of $s_i$ and $y_i^*$ is described by
...
Dark Fairy arrives and makes her black magic!
Sleeping Beauty awakens and observes the modified sample $y = (1, 1, 0, 1, 1, 0)$.

\begin{enumerate}
  \item If possible estimate $a$ by maximum likelihood, if
  Dark Fairy replaces all $y_i^*$ by $0$ where $s_i = 1$.
  \item If possible estimate $a$ by maximum likelihood, if
  Dark Fairy removes all observations with $s_i = 1$.
  \item If possible estimate $a$ by maximum likelihood, if
  Dark Fairy does nothing.
\end{enumerate}
}
...
\begin{problem}
\trru{
  Регрессор $x_i$ равновероятно принимает два значения, $x_i = 1$ или $x_i = 2$.
  Условные распределения вектора $(s_i, y_i^*)$ заданы парой табличек

\begin{tabular}{ccc}
\toprule
$(s_i, y_i^* \mid x_i = 1)$ & $y_i^* = 1$ & $y_i^* = 2$ \\
\midrule
$s_i = 0$ & $2a$ & $1 - 6a$ \\
$s_i = 1$ & $3a$ & $a$ \\
\bottomrule
...
Мы знаем значение $y_i^*$, если $s_i = 1$, и не знаем $y_i^*$, если $s_i = 0$.
Для удобства можно считать, что вместо $y_i^*$ мы наблюдаем $y_i = s_i \cdot y_i^*$.
Мы всегда наблюдаем $s_i$ и $x_i$.


\begin{enumerate}
  \item Найдите $\E(y_i^* \mid x_i)$.
  \item Найдите $\E(y_i \mid x_i)$.
  \item Найдите $\E(y_i^* \mid x_i, s_i = 1)$.
\end{enumerate}

Рассмотрим наблюдаемую выборку $(x_1 = 1, y_1 = 0)$, $(x_2 = 2, y_2 = 1)$, $(x_3 = 1, y_3 = 1)$.

\begin{enumerate}[resume]
  \item Найдите оценку $a$ методом максимального правдоподобия.
  \item Найдите оценку $a$ используя только значение $x_i$ и $s_i$ для каждого наблюдения. 
  \item Найдите оценку $a$ используя только значение $s_i$ для каждого наблюдения. 
\end{enumerate}
}
...
\section{\trru{Эндогенность}\tren{Endogeneity}}


\trru{Прочесть про инструментальные переменные можно, например, в \cite{kurt2021notes}.}
\tren{A good source on instrumental variables is \cite{kurt2021notes}.}
...
Величины $x_i$, $a_i$ и $u_i$ независимы и нормально распределены.
...
\item Найдите $\E(\hb_2 \mid x^*)$ и $\E(\hb_2)$.
\item Являются ли оценки, получаемые Агнессой, несмещённым?
\end{enumerate}
}
\tren{
  Economaîtresse Agnesse would like to estimate the model $y_i=\beta_1+\beta_2 x_i +u_i$.
  Unfortunately the variable $x_i$ is unobserved. 
  Instead she observes  $x_i^*$. 
  The variable $x_i^*$ includes a measurement error, $x_i^*=x_i+a_i$. 
  Variances are known $\Var(x_i)=9$, $\Var(a_i)=4$,   $\Var(u_i)=1$. 
  Variables $x_i$, $a_i$ and $u_i$ are normally distributed and independent.
  Observations are independent.
  
  Agnesse estimates uses ordinary least squares for the regression $\hy_i=\hb_1+\hb_2 x_i^*$.
  \begin{enumerate}
  \item Find $\plim \hb_2$.
  \item Is the estimator consistent?
  \item Find $\E(\hb_2 \mid x^*)$ and $\E(\hb_2)$.
...
\begin{sol}
  ...
  \]
  \trru{Оценка $\hb_2$ несостоятельна и смещена.}
  \tren{The estimator $\hb_2$ is biased and unconsistent.}
\end{sol}
\end{problem}
...
\begin{problem}
\tren{Let $z_i$ be binary variable. }
\begin{enumerate}
  \item \tren{Prove that } \trru{Докажите, что }
  \[
  ...
  \]
  \tren{where $\bar x_1$ is the average value of $x$ when $z=1$, $\bar x_0$ is the average value of $x$ when $z=0$, and same notation applies for $\bar y_1$ and $\bar y_0$.}
  \trru{где $\bar x_1$ — это среднее значение $x$ при $z=1$, $\bar x_0$ — это среднее $x$ при $z=0$, и аналогичный смысл несут обозначения $\bar y_1$ и $\bar y_0$.}
\end{enumerate}
\begin{sol}
\end{sol}
\end{problem}
