...
\begin{problem}
Найдите на картинке все перпендикулярные векторы. 
...
Для каждого из данных случаев найдите матрицу-шляпницу $H$, проецирующую на пространство столбцов матрицы регрессоров $X$, вектор $\hy$, $RSS$, $ESS$, $TSS$:}
\tren{We have $n$ observations. 
For each case find the hat-matrix $H$ that projects onto the column space of regressor matrix $X$, vector $\hy$, $RSS$, $ESS$, $TSS$:}
